...
$coOrdinators = mysqli_num_rows(mysqli_query($conn, "SELECT ROLLNO FROM coordinator"));
$events       = mysqli_num_rows(mysqli_query($conn, "SELECT EVENTS FROM event WHERE EVENTS!='$sympName'"));
$eventOpen    = mysqli_num_rows(mysqli_query($conn, "SELECT EVENTS FROM event WHERE EVENTS!='$sympName' AND STATUS='OPEN'"));
	...
	<body class="admin-panel">
		...
		<div class="wrapper">
			...
			<div class="container">
				...
				<div class="card">
					<p class="count"><?php echo $events; ?></p>
					<span class="type">Total Events</span>
				</div>
				<div class="card">
					<p class="count"><?php echo $eventOpen; ?></p>
					<span class="type">Opened Events</span>
				</div>
				<div class="card">
					<p class="count"><?php echo $coOrdinators; ?></p>
					<span class="type">Co-Ordinators</span>
				</div>
			</div>
		...
		<script>
			$(document).ready(function() {
				...
				window.setTimeout(function() {
					$(".navigation-admin > ul > li.home").addClass("active");
				}, 700);
			});
		</script>
	</body>
